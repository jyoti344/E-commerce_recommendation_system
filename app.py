...
user_book_recomendation = dill.load(
    open("./pickle_files/user_book_recomend.pkl", "rb")
)

search_book_recomendation = dill.load(
    open("./pickle_files/model_book_recomend.pkl", "rb")
)

also_bought = dill.load(
    open("./pickle_files/get_also_bought.pkl", "rb")
)
...
def get_book_details(book_list):
    ...
    filtered_df = main_df[
        main_df['Book-Title'].isin(book_list)
    ][
        [
            'Book-Title',
            'Book-Rating',
    ...
    filtered_df['Book-Title'] = pd.Categorical(
...
def home():
    ...
    df = user_book_recomendation(
        user_id=login_id,
        similar_user_ids=related_ids,
        retings_with_book=main_df,
        n_recommendation=12
    )
...
def search():
    ...
    book_distances, book_indices = book_knn.kneighbors(
        query_vector,
        n_neighbors=13
    )

    book_ids = search_book_recomendation(
        book_indices,
        n_recommendation=13
    )
